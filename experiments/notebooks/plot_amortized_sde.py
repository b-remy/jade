"""Amortized posterior plots driven by a STOCHASTIC (SDE) sampler.

This is the stochastic-sampler counterpart of ``plot_amortized.py``. Instead of
integrating the deterministic probability-flow ODE (Euler/Heun), it turns the
*same frozen* linear-interpolant flow model into a stochastic sampler using the
generative SDE of Domingo-Enrich et al. 2024 ("Adjoint Matching",
arXiv:2409.08861), Eqs. 10-11, with the **memoryless** noise schedule.

Derivation for our interpolant ``xt = t*x + (1-t)*noise`` (t=0 noise -> t=1 data),
which maps to the paper's ``X_t = beta_t X_0 + alpha_t X_1`` with
``alpha_t = t, beta_t = 1-t`` (so ``alpha_dot = 1, beta_dot = -1``):

    kappa_t = alpha_dot/alpha = 1/t
    eta_t   = beta_t^2 (alpha_dot/alpha - beta_dot/beta) = (1-t)/t
    score   s(x,t) = (v - kappa_t x)/eta_t = (t*v - x)/(1-t)        [= -x0_hat/(1-t)]

Paper Eq. 10 generative SDE (forward in t, X_0 ~ N(0,I)):

    dX = b(x,t) dt + sigma(t) dB,   b = v(x,t) + (sigma^2/2) s(x,t)

Memoryless schedule (Eq. 11): sigma^2 = 2 eta_t = 2(1-t)/t. The score's 1/(1-t)
then cancels and the drift reduces to

    b(x,t) = 2 v(x,t) - x/t,   sigma(t) = sqrt(2(1-t)/t).

A stochasticity knob ``noise_scale = g`` generalizes this (g=0 -> deterministic
ODE, g=1 -> memoryless SDE):

    b(x,t) = (1+g) v - g * x/t,   sigma(t) = sqrt(2 g (1-t)/t).

Divergence handling: the only blow-up is the 1/t factor at t->0, clipped with
``t_eps = 0.05`` via ``t_lo = clip(t, t_eps)``. The score's 1/(1-t) singularity at
t->1 cancels analytically in the memoryless drift, so (1-t) is NOT clipped.
"""
...
# Sampler-side t_eps override (clip for the 1/(1-t) inside v_pred). The SDE
# sampler below uses its own t_eps for the 1/t terms in the schedule.
model.t_eps = 0.05
print(f"Sampler t_eps overridden to {model.t_eps}")


# =============================================================================
# Stochastic (SDE) sampler: Adjoint-Matching Eq. 10/11, memoryless schedule.
# Mirrors the jade.sampling.Sampler call signature so it drops straight into the
# existing ``jax.vmap(sampler, in_axes=(0,0,None,0))`` call below.
# =============================================================================
class AdjointMatchingSDESampler(nnx.Module):
    ...
    ``g=1`` is the memoryless schedule (Eq. 11); ``g=0`` recovers the
    deterministic Euler ODE. The ``1/t`` factor is clipped with ``t_eps``; the
    ``1/(1-t)`` of the score cancels in the memoryless drift and is not clipped.
    """

    def __init__(self, model, num_steps: int = 256, t_eps: float = 0.05,
...
def sample(obs, key, states=states, batch_size=128):
    ...
    sampler = AdjointMatchingSDESampler(
        model=model, num_steps=1000, t_eps=0.05, noise_scale=1.0
    )
...
samples_posterior = MCSamples(samples=np.array(theta_posterior), names=names, label="JADE (SDE)")
...
fig.supxlabel(r'$\kappa$', fontsize=fontsize_text, y=-0.08)
