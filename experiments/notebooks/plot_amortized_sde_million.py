...
# SDE-sampler mirror of plot_amortized.py for the 1e6-sample model fk49rnft.
# Identical plots; only the model, sampler (memoryless SDE, 1000 Euler steps),
# and save_dir differ.
save_dir = 'amortized_sde_million'
...
cfg, states = load_model(
    ...
    # "/u/bremy/repos/jade/experiments/wandb/run-20260504_100148-by4dv8sg/files/checkpoints",
     ...
     "/u/bremy/repos/jade/experiments/wandb/run-20260615_153845-fk49rnft/files/checkpoints",
     "JADE_B_16_ema_best"
)
...
class AdjointMatchingSDESampler(nnx.Module):
    ...
    Copied from plot_amortized_sde.py (Domingo-Enrich et al. 2024, Eqs. 10-11).
    """
...
def sample(obs, key, states=states, batch_size=128):
    ...
    sampler = AdjointMatchingSDESampler(model=model, num_steps=1000, t_eps=0.05, noise_scale=1.0)
...
for i in range(5):
    ...
    ax[i].fill_between(grid, pdf_mean - pdf_std, pdf_mean + pdf_std,
                       color='tab:blue', alpha=0.3)
    ax[i].set_title(f'bin {i + 1}', fontsize=fontsize_ticks)
