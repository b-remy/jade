...
def load_to_gpu(samples_dir, space, device, max_jobs=None):
    ...
    q_field = 0
    if space == "joint":
        x0 = np.load(os.path.join(samples_dir, f"x_samples_job_{ids[0]}.npy"), mmap_mode="r")
    ...
    print(
        f"Allocating {space} posterior on {device}: "
        f"(1, {T_total}, {S}, {q_total}) float32 = "
        f"{T_total * S * q_total * 4 / 2**30:.3f} GiB "
        f"(q_cosmo={q_cosmo}, q_field={q_field})"
    )
    posterior_gpu = torch.empty((1, T_total, S, q_total), dtype=torch.float32, device=device)
    truth_gpu = torch.empty((T_total, q_total), dtype=torch.float32, device=device)
    ...
    for jid in ids:
        ...
        if space == "joint":
            ...
            posterior_gpu[0, start:end, :, q_cosmo:].copy_(torch.from_numpy(x.reshape(obs_per_job, S, q_field)))
            truth_gpu[start:end, q_cosmo:].copy_(torch.from_numpy(tx.reshape(obs_per_job, q_field)))
            del x, tx
...
def main():
    ...
    parser.add_argument(
        "--num-runs", type=int, default=1, help="MC runs per bootstrap iter (mira_bootstrap reference uses 1)."
    )
    parser.add_argument(
        "--save-npz", default=None, help="Defaults to mira_<space>_scores.npz next to the samples' parent directory."
    )
    ...
    t = time.time() - t0
    m = float(boot_mean[0].cpu())
    s = float(boot_std[0].cpu())
    print(
        f"mira {args.space} score: {m:.6f} +/- {s:.6f}   "
        f"(wall {t:.1f}s, {args.num_bootstrap} boots × num_runs={args.num_runs})"
    )
    print(f"SE = {s / np.sqrt(args.num_bootstrap):.6f}")
    np.savez(save_npz, mean=m, std=s, q_cosmo=q_cosmo, q_field=q_field, norm=args.norm)
