...
from jade.utils import (dump_model, load_config, load_model, plot_corner,
                        plot_denoiser, plot_samples)
...
def normalize_batch(batch):
    return {
        "map": (batch["map"] - FIELD_MEAN.reshape(1, 1, 1, -1))
        / FIELD_STD.reshape(1, 1, 1, -1),
...
def make_cond(x, key):
    ...
    noisy = x * std + mean + sigma_lsst.reshape((1, 1, 1, -1)) * jax.random.normal(
        key, shape=x.shape)
...
def create_optimizer(cfg, total_steps):
    ...
    return optax.chain(
        ...
        optax.adamw(learning_rate=lr, b1=opt['beta1'], b2=opt['beta2'],
...
def log_visuals(cfg, model, key, x_val, cosmo_val, corner):
    ...
    if corner is not None and use_cond:
        ...
        _, cosmo_post = jax.vmap(sampler, in_axes=(0, 0, None, 0))(
            x_0, cosmo_0, cond, jax.random.split(keys[2], n))
...
def train(cfg):
    ...
    mesh = (Mesh(mesh_utils.create_device_mesh((num_devices,)), axis_names=('fsdp',))
            if use_sharding else None)
    print(f"{num_devices} device(s) on {jax.devices()[0].platform}, "
          f"FSDP {'on' if use_sharding else 'off'}")

    if cfg['training']['use_mixed_precision']:
        jax.config.update('jax_default_matmul_precision', 'bfloat16')

    run = wandb.init(project=cfg['logging']['project'],
                     entity=cfg['logging']['entity'], config=cfg)
    ...
    dataset = dataset.select_columns(
        [c for c in ("map", "theta") if c in dataset.column_names])
    ...
    for epoch in range(num_epochs):
        for batch in tqdm(train_loader, desc=f"epoch {epoch + 1}/{num_epochs}"):
            ...
            if use_ema and ema_params is None and epoch >= 1 \
                    and not cfg['start_from_checkpoint']:
                ema_params = jax.tree.map(lambda a: a.copy(),
                                          nnx.state(model, nnx.Param))
                if mesh is not None:
                    ema_params = shard_state(ema_params, mesh)
                print(f"EMA initialized at epoch {epoch}")
            if use_ema and ema_params is not None:
                ema_params = update_ema(ema_params, nnx.state(model, nnx.Param),
                                        cfg['ema']['decay'])
        ...
        run.log({"val/loss_total": val_loss, "val/v_loss_field": val_loss_x,
                 "val/v_loss_cosmo": val_loss_cosmo, "epoch": epoch + 1})
        ...
        if (epoch + 1) % cfg['checkpoint']['save_every_n_epochs'] == 0:
            ...
            save_checkpoints(cfg, model, ema_params if use_ema else None,
                             checkpoint_dir, best)
