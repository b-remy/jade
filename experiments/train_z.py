...
def normalize_batch(batch):
    ...
    return {'z': z, 'theta': theta_norm, 'map': map_norm}
...
def train(cfg):
    ...
    dataset = dataset.filter(
        lambda example: (
            not np.isnan(example["z"]).any()
            and not np.isnan(example["map"]).any()
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(loader, desc=f"Epoch {epoch+1}/{cfg['training']['num_epochs']}"):
            ...
            x = augment(batch["z"], jax.random.split(subkey, len(batch["z"])))
            cond = augment(batch["map"], jax.random.split(subkey, len(batch["map"])))
            cosmo = batch["theta"] * cfg['loss']['SCALE_COSMO']
            
            key, subkey = jax.random.split(key, 2)
            # Noisy field condition
            if cfg["model"]["enable_cond_image"]:
                cond_field = make_cond(cond, subkey)
            else:
                cond_field = None
        ...
        for batch in val_loader:
            ...
            if cfg["model"]["enable_cond_image"]:
                cond_val = make_cond(cond_val, val_key)
            else:
                cond_val = None
