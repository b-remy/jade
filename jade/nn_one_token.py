...
class JADE(nnx.Module):
    ...
    def __init__(
        ...
        # RoPE WITHOUT conditioning (1 cosmo token as prefix instead of cosmo_dim)
        self.feat_rope_nocond = VisionRotaryEmbeddingFast(
            dim=half_head_dim,
            pt_seq_len=hw_seq_len,
            num_cls_token=1,  # Changed from cosmo_dim to 1
            rngs=rngs_rope_nocond
        )

        # RoPE WITH conditioning (1 cosmo token as prefix)
        if self.enable_cond_image:
