...
class BottleneckPatchEmbed(nnx.Module):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=1, pca_dim=768, embed_dim=768, bias=True, rngs=None):
        ...
        self.proj2 = nnx.Conv(
            in_features=pca_dim, out_features=embed_dim, kernel_size=(1, 1), strides=(1, 1), use_bias=bias, rngs=rngs
        )
...
def scaled_dot_product_attention(query, key, value, dropout_p=0.0, dropout_key=None, attn_mask=None, attn_impl="dense"):
    """
...
class Attention(nnx.Module):
    ...
    def __call__(self, x, rope, train=False, key=None, attn_mask=None):
        ...
        if self.split_qkv:
            ...
            qkv = self.qkv(x)
        qkv = rearrange(qkv, "N (three H D) -> three H N D", three=3, H=self.num_heads, D=self.head_dim)
...
class FinalLayer(nnx.Module):
    ...
    def __init__(self, hidden_size, patch_size, out_channels, rngs=None):
        ...
        self.linear = nnx.Linear(hidden_size, patch_size * patch_size * out_channels, use_bias=True, rngs=rngs)
...
class MultiResolutionRoPE(nnx.Module):
    ...
    def __init__(self, dim, field_seq_len, cond_seq_len=0, num_nonspatial_tokens=0, theta=10000, rngs=None):
        """
        Args:
...
class CosmologyEmbedder(nnx.Module):
    ...
    def __init__(self, cosmo_dim, hidden_size, num_tokens=16, rngs=None):
        ...
        self.token_pos_embed = nnx.Param(jax.random.normal(rngs(), (num_tokens, hidden_size)) * 0.02)
...
class JADE(nnx.Module):
    def __init__(
        ...
        idx = 0
        rngs_t_embedder = nnx.Rngs(rng_keys[idx])
        idx += 1
        rngs_x_embedder = nnx.Rngs(rng_keys[idx])
        idx += 1
        rngs_cosmo_embedder = nnx.Rngs(rng_keys[idx])
        idx += 1

        if enable_cond_image:
            rngs_cond_embedder = nnx.Rngs(rng_keys[idx])
            idx += 1

        rngs_rope = nnx.Rngs(rng_keys[idx])
        idx += 1
        rngs_rope_nocond = nnx.Rngs(rng_keys[idx])
        idx += 1
        rngs_final = nnx.Rngs(rng_keys[idx])
        idx += 1
        rngs_cosmo_head = nnx.Rngs(rng_keys[idx])
        idx += 1
        ...
        pos_embed = get_2d_sincos_pos_embed(hidden_size, int(num_field_patches**0.5), cls_token=False, extra_tokens=0)
        ...
        self.cosmo_head = CosmologyPredictor(hidden_size, cosmo_dim, num_tokens=num_cosmo_tokens, rngs=rngs_cosmo_head)
    ...
    def initialize_weights(self):
        def init_linear_xavier(module):
            if isinstance(module, nnx.Linear):
                ...
                w_init = jax.nn.initializers.xavier_uniform()(jax.random.PRNGKey(0), w_flat.shape)
        ...
        if self.enable_cond_image:
            ...
            w1_c_init = jax.nn.initializers.xavier_uniform()(
                jax.random.PRNGKey(10), w1_c.reshape(w1_c.shape[0], -1).shape
            )
            self.cond_embedder.proj1.kernel.value = w1_c_init.reshape(w1_c.shape)
            w2_c = self.cond_embedder.proj2.kernel.value
            w2_c_init = jax.nn.initializers.xavier_uniform()(
                jax.random.PRNGKey(11), w2_c.reshape(w2_c.shape[0], -1).shape
            )
        ...
        self.t_embedder.linear1.kernel.value = (
            jax.random.normal(jax.random.PRNGKey(3), self.t_embedder.linear1.kernel.value.shape) * 0.02
        )
        self.t_embedder.linear2.kernel.value = (
            jax.random.normal(jax.random.PRNGKey(4), self.t_embedder.linear2.kernel.value.shape) * 0.02
        )

        # Cosmo embedder init
        self.cosmo_embedder.proj.kernel.value = (
            jax.random.normal(jax.random.PRNGKey(5), self.cosmo_embedder.proj.kernel.value.shape) * 0.02
        )
        self.cosmo_embedder.proj.bias.value = jnp.zeros_like(self.cosmo_embedder.proj.bias.value)
        self.cosmo_embedder.token_pos_embed.value = (
            jax.random.normal(jax.random.PRNGKey(6), self.cosmo_embedder.token_pos_embed.value.shape) * 0.02
        )
    ...
    def __call__(self, field, cosmo, t, cond=None, train=False, key=None):
        ...
        for i, block in enumerate(self.blocks):
            ...
            x = block(x, c, feat_rope=rope, train=train, key=block_keys[i], attn_mask=attn_mask)
...
def convert_state_split_qkv(state):
    ...
    blocks_state = new_state["model"]["blocks"] if "model" in new_state else new_state["blocks"]
...
class JiT(nnx.Module):
    ...
    def __init__(
        ...
        pos_embed = get_2d_sincos_pos_embed(hidden_size, int(num_patches**0.5), cls_token=False, extra_tokens=0)
        self.pos_embed = nnx.Param(jnp.array(pos_embed, dtype=jnp.float32))

        if self.in_context_len > 0:
            self.in_context_posemb = nnx.Param(jax.random.normal(rngs(), (self.in_context_len, hidden_size)) * 0.02)
    ...
    def __call__(self, x, t, y, train=False):
        ...
        for i, block in enumerate(self.blocks):
            if self.in_context_len > 0 and i == self.in_context_start:
                in_context_tokens = jnp.tile(jnp.expand_dims(y_emb, 0), (self.in_context_len, 1))
