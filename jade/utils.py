...
def plot_denoiser(x, cosmo, model, key, cfg):
    ...
    gs = fig.add_gridspec(3, 6, width_ratios=[1, 1, 1, 1, 1, 0.5], hspace=0.3, wspace=0.2)
...
def plot_samples(x_samples, cosmo_samples, n_samples=6, denormalize=True):
    ...
    for i in range(n_samples):
        ...
        for j in range(n_channels):
            ...
            # Plot field channel
            ax.imshow(x_display[i, ..., j], cmap="viridis", aspect="auto")
            ax.axis("off")
    ...
    plt.suptitle("Generated Samples: Density Fields + Cosmological Parameters", fontsize=14, fontweight="bold", y=0.995)
